...
def plot_prob(csv, player_id, df, show=False):
    ...
    for i, row in df.iterrows():
        ...
        plt.title(date.capitalize() + ' ' + session.capitalize() + ' ' + player_id.capitalize() + ' Probabilidad - Decisión ' + str(i % 10))
        ...
        ax.text(row['bx'] + 5, row['by'] + 5, 'Resultado B: ' + str(row['prob_b']) + '%')

        if show == True:
            plt.show()
        else:
            plt.savefig(date.capitalize() + '_' + session.capitalize() + '_'
                + player_id.capitalize() + '_Probabilidad_Decisión-' + str(i % 10) + '.png', format='png', dpi=250)
            ...
            if display == 'coded' and mode != 'det_giv':
                ...
                for _, row in df.iterrows():
                    if row['prob_a'] not in probs:
                        ...
                        leg_key.append('Estado A: ' + str(row['prob_a']) + '%')
                        i += 1
